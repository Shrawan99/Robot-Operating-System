...
//added by Rakesh Shrestha
int DynamixelClass::setPositionControl(int ID){
...
int DynamixelClass::setAngleLimit(unsigned char ID, int CWLimit, int CCWLimit)
{
    ...
    CCW_H = CCWLimit >> 8;
    CCW_L = CCWLimit;  
    //correction in checksum (made by Rakesh Shrestha)
	//Checksum = (~(ID + AX_VL_LENGTH +AX_WRITE_DATA+ AX_CW_ANGLE_LIMIT_L + CW_H + CW_L + AX_CCW_ANGLE_LIMIT_L + CCW_H + CCW_L))&0xFF;//AX_CCW_ANGLE_LIMIT_L is sent twice, AX_VL_LENGTH is used???
	Checksum = (~(ID + AX_CCW_CW_LENGTH +AX_WRITE_DATA+ AX_CW_ANGLE_LIMIT_L + CW_H + CW_L +  CCW_H + CCW_L))&0xFF;
    ...
    sendData(CW_L);
	sendData(CW_H);
	//sendData(AX_CCW_ANGLE_LIMIT_L);//why'd you send this twice!!! -> Rakesh Shrestha
    sendData(CCW_L);
	sendData(CCW_H);
	sendData(Checksum);
	delayus(TX_DELAY_TIME);
